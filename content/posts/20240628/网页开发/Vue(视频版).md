...
​			备注：Vue中有很多的指令，且形式都是： v-????，可多层表示

# 三、数据绑定

- 1.单向绑定(v-bind)：数据只会从data流向页面

- 2.双向绑定(v-model)：数据不仅能从data流向页面，还可以从页面流向data

  - 备注：

  1. 双向绑定一般都应用在表单类元素(如：input、select等)
  2. v-mode:value可以简写为v-model，因为v-model默认收集就是value值
...
- 3.一个重要原则
  ...
  - 由Vue管理的函数，一定不要写箭头函数，一旦写了箭头函数，this将是window，不是Vue实例了
...
4. methods中配置的函数，都是被Vue所管理的函数，this的只想是vm或组件实例对象
...
2. watch能完成的功能，computed不一定能完成，例如：watch能进行异步操作。
   ...
   1. 所被Vue管理的函数，最好写成普通函数，这样yhis的指向才是vm 或 组件实例对象

   2. 所有不被Vue所管理的函数（定时器的回调函数，ajax的回调函数等），最好携程箭头函数。
...
- :style样式="{fontSize: xxx}"其中xxx是动态值
...
- 随后Vue进行【新虚拟DOM】与【I旧虚拟DOM】的差异比较，比较规则如下：
...
特别注意：Vue.set() 和 vm.$set() 不能给vm或vm的跟数据对象添加属性！！！
...
4. 关于this指向：
   ...
   data函数、methods中的函数、watch中的函数、computed中的函数 它们的this均是【Vue实例对					  象】。
...
6. 因为组件是可复用的 Vue 实例，所以它们与 new Vue 接收相同的选项，例如 data、computed、watch、methods 以及生命周期钩子等。仅有的例外是像 el 这样根实例特有的选项。
   所以，vm与vc属性配置并不是一模一样，尽管vc底层复用了很多vm的逻辑
...
>
> ​		需要使用**render函数**接收到的createElement函数去指定具体内容。
